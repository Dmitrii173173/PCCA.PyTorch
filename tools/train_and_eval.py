...
# Import the network module based on the provided argument
...
def train_one_epoch(model, train_loader, optimizer, epoch, lr_scheduler, total_it, tb_log, log_f):
    ...
    for it, batch in enumerate(train_loader):
        ...
        pred_class = (torch.sigmoid(pred_cls) > FG_THRESH).long()
        ...
        # Calculate accuracy
        accuracy = (pred_class == cls_labels).float().mean()
        ...
        if tb_log is not None:
            ...
            tb_log.log_value('train_accuracy', accuracy, total_it)

        log_print('training epoch %d: it=%d/%d, total_it=%d, loss=%.5f, fg_iou=%.3f, accuracy=%.3f, lr=%f' %
                  (epoch, it, len(train_loader), total_it, loss.item(), iou.item(), accuracy.item(), cur_lr), log_f=log_f)
...
def eval_one_epoch(model, eval_loader, epoch, tb_log=None, log_f=None):
    ...
    for it, batch in enumerate(eval_loader):
        ...
        log_print('EVAL: it=%d/%d, iou=%.3f, accuracy=%.3f' % (it, len(eval_loader), iou, accuracy), log_f=log_f)

    iou_list = np.array(iou_list)
    accuracy_list = np.array(accuracy_list)
    avg_iou = iou_list.mean()
    avg_accuracy = accuracy_list.mean()
    if tb_log is not None:
        tb_log.log_value('eval_fg_iou', avg_iou, epoch)
        tb_log.log_value('eval_accuracy', avg_accuracy, epoch)

    log_print('\nEpoch %d: Average IoU (samples=%d): %.6f, Average Accuracy: %.6f' % (epoch, len(iou_list), avg_iou, avg_accuracy), log_f=log_f)
...
def train_and_eval(model, train_loader, eval_loader, tb_log, ckpt_dir, log_f):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.ckpt_save_interval == 0:
            with torch.no_grad():
                avg_iou, avg_accuracy = eval_one_epoch(model, eval_loader, epoch, tb_log, log_f)
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        model.cuda()
        with torch.no_grad():
            avg_iou, avg_accuracy = eval_one_epoch(model, eval_loader, epoch)
    else:
