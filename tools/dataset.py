...
def load_clusters(cluster_dir, sample_id):
    cluster_file = os.path.join(cluster_dir, '%06d.npy' % sample_id)
    if not os.path.exists(cluster_file):
        raise FileNotFoundError(f"Cluster file {cluster_file} does not exist")
    clusters = np.load(cluster_file, allow_pickle=True)
    return clusters
...
class KittiDataset(torch_data.Dataset):
    def __init__(self, root_dir, split='train', mode='TRAIN'):
        ...
        self.cluster_dir = os.path.join(self.imageset_dir, 'cluster')
    ...
    def __getitem__(self, index):
        ...
        clusters = load_clusters(self.cluster_dir, sample_id)
        sample_info = {'sample_id': sample_id, 'clusters': clusters}
    ...
    def collate_batch(self, batch):
        ...
        # Добавление кластеров в батч
        if 'clusters' in batch[0]:
            ans_dict['clusters'] = [batch[k]['clusters'] for k in range(batch_size)]
