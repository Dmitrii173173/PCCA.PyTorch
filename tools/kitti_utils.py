...
def load_clusters_from_txt(cluster_file):
    clusters = []
    if os.path.exists(cluster_file):
        ...
        for line in lines:
            ...
            elif line.startswith("Bounding Box Min"):
                current_cluster["bounding_box_min"] = list(map(float, line.split(":")[1].strip()[1:-1].split(",")))
            elif line.startswith("Bounding Box Max"):
                current_cluster["bounding_box_max"] = list(map(float, line.split(":")[1].strip()[1:-1].split(",")))
            elif line.startswith("Centroid"):
                current_cluster["centroid"] = list(map(float, line.split(":")[1].strip()[1:-1].split(",")))
            elif line.startswith("Point Count"):
                current_cluster["point_count"] = int(line.split(":")[1].strip())
        if current_cluster:
            clusters.append(current_cluster)
    return clusters

def convert_txt_to_npy(cluster_dir):
    for file_name in os.listdir(cluster_dir):
        if file_name.endswith('.txt'):
            ...
            clusters = load_clusters_from_txt(cluster_file)
            npy_file = os.path.join(cluster_dir, f'{sample_id:06d}.npy')
            np.save(npy_file, clusters)

convert_txt_to_npy('./data/KITTI/object/training/cluster/')
